...
<div class="max-w-2xl space-y-6">
    ...
    <div class="flex items-center space-x-2">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white tracking-tight">Edit Testimonial</h1>
            <p class="text-zinc-400 text-sm mt-1">Ubah data ulasan dari klien Anda.</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="message" id="message" rows="4" required
                    class="w-full px-4 py-2 bg-zinc-950/60 border border-zinc-800 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition duration-200"
                    placeholder="Write client testimonial here...">{{ old('message', $testimonial->message) }}</textarea>
            </div>
                    ...
                    <div class="w-16 h-16 rounded-full bg-zinc-950 border border-zinc-800 flex items-center justify-center shrink-0 overflow-hidden">
                        ...
                        <template x-if="!photoPreview">
                            @if($testimonial->client_photo)
                                <img src="{{ asset('storage/' . $testimonial->client_photo) }}" class="w-full h-full object-cover">
                            @else

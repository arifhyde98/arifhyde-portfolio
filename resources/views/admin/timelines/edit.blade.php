...
<div class="max-w-2xl space-y-6">
    ...
    <div class="flex items-center space-x-2">
        <a href="{{ route('admin.timelines.index') }}" class="text-zinc-400 hover:text-white transition duration-200">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-white tracking-tight">Edit Milestone</h1>
            <p class="text-zinc-400 text-sm mt-1">Ubah data pencapaian Anda.</p>
        </div>
    </div>
    ...
    <div class="bg-zinc-900/40 backdrop-blur-xl border border-zinc-900 rounded-xl p-6 lg:p-8">
        ...
        <form action="{{ route('admin.timelines.update', $timeline) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Year -->
                <div>
                    <label for="year" class="block text-sm font-medium text-zinc-300 mb-1.5">Year / Period</label>
                    <input type="text" name="year" id="year" value="{{ old('year', $timeline->year) }}" required
                        ...
                        placeholder="e.g. 2026 or Jan - Jun 2026">
                </div>

                <!-- Display Order -->
                <div>
                    <label for="display_order" class="block text-sm font-medium text-zinc-300 mb-1.5">Display Order</label>
                    <input type="number" name="display_order" id="display_order" value="{{ old('display_order', $timeline->display_order) }}" required
                        class="w-full px-4 py-2 bg-zinc-950/60 border border-zinc-800 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition duration-200">
                </div>
            </div>
            ...
            <!-- Title -->
            <div>
                <label for="title" class="block text-sm font-medium text-zinc-300 mb-1.5">Milestone Title</label>
                <input type="text" name="title" id="title" value="{{ old('title', $timeline->title) }}" required
                    class="w-full px-4 py-2 bg-zinc-950/60 border border-zinc-800 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition duration-200"
                    placeholder="e.g. Built Laravel-based CRUD systems">
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                    class="w-full px-4 py-2 bg-zinc-950/60 border border-zinc-800 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition duration-200"
                    placeholder="Describe what you achieved or learned in this period...">{{ old('description', $timeline->description) }}</textarea>
            </div>
            ...
            <div class="flex justify-end space-x-3 pt-4 border-t border-zinc-900">
                <a href="{{ route('admin.timelines.index') }}" class="px-4 py-2.5 bg-zinc-950 border border-zinc-800 hover:border-zinc-700 text-zinc-300 hover:text-white rounded-lg text-sm font-semibold transition duration-200">
                    Cancel
                </a>
                <button type="submit"
